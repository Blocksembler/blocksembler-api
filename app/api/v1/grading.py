...
from app.config import MESSAGE_QUEUE_URL, MESSAGE_QUEUE_PASSWORD, MESSAGE_QUEUE_USER, GRADING_RESPONSE_QUEUE_TTL
...
async def submit_grading_job(job_msg: dict, session: AsyncSession):
...
async def create_submission(new_submission: ExerciseSubmission, session: AsyncSession = Depends(get_session)) -> str:
    ...
    try:
        ...
        await submit_grading_job(job_msg, session)
        ...
        raise HTTPException(status_code=500, detail="Scheduling a grading job failed.")
